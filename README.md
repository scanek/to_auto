...
Текущая версия: **v2.9.1 (Clean Initial State, Universal JSON Backup Import & Standalone Android App)**
...
### 🏢 4. Гараж 2.0 и профиль автомобиля
* **Минималистичный быстрый интерфейс**:
  * Вся карточка автомобиля открывается в 1 клик/тап без лишних громоздких кнопок.
  * Убран визуальный шум — внимание сосредоточено на актуальных параметрах машины.
  * **Компактный умный бейдж регламентов ТО**: предупреждает о приближении или просрочке регламента без загромождения экрана.
...
### 📱 16. Полностью автономное мобильное приложение для Android (Capacitor & Local DB Engine)
* **Чистый старт без демонстрационных авто**:
  * При первом запуске гараж открывается пустым с удобными кнопками **«+ Добавить автомобиль»** или **«📥 Восстановить из бэкапа JSON»**.
* **Универсальный импорт резервных копий из веб-версии**:
  * Поддержка восстановления файлов бэкапа любого формата: бэкапы одиночного автомобиля, полный архив базы данных (`bortovoi_full_backup.json`), выгрузки из автономного приложения.
  * Автоматическая конвертация и сопоставление всех сущностей: регламенты ТО, выполненные работы, заправки, комплекты шин и дисков, страховки и документы.
* **100% Offline-First хранение на устройстве**:
  * Приложение не требует сервера, интернета и Docker.
  * Все данные об автомобилях, сервисных книжках, заправках, шинах, документах и расходах сохраняются во встроенной локальной базе данных смартфона.
* **Встроенный математический движок аналитики**:
  * Локальный расчет стоимости владения (**TCO**), себестоимости 1 км пути и расхода топлива прямо на телефоне.
  * Экспорт и импорт полной резервной копии (**JSON**), сохранение сервисной книжки в PDF и Excel.
* **Нативная интеграция Android**:
  * Обработка аппаратной кнопки «Назад» (закрытие окон, навигация).
  * Адаптация под вырезы дисплеев (Safe Area Insets) и стилизация Status Bar.
  * Поддержка камеры для фото чеков, запчастей и одометра.
* **Гибридный режим синхронизации**:
  * При желании в окне настроек можно в любой момент переключиться на свой домашний сервер AutoTracker для облачной синхронизации.
* **📥 Установка и сборка APK**:
  * **Готовый APK:** доступен для скачивания в разделе [Releases](https://github.com/scanek/to_auto/releases) (файл `AutoTracker-v2.9.1.apk`).
  * **Сборка из исходников:**
    ...
    # Готовый файл: frontend/android/app/build/outputs/apk/debug/app-debug.apk
    ```
